...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, label_data, image_folder=None, transform=None):
        ...
        **Example**::
        ...
        self._csv = preprocessing_image_id(self._csv)
...
class Model:
    ...
    def fit(self, dataset, val_dataset=None, epochs=10, learning_rate=0.005, momentum=0.9,
            weight_decay=0.0005, gamma=0.1, lr_step_size=3, verbose=True, optimizer = None, writer = None):
        ...
        **Example**::
        ...
        #if not optimizer: optimizer = torch.optim.SGD(parameters, lr=learning_rate, momentum=momentum, weight_decay=weight_decay)

        optimizer = eval("torch.optim."+optimizer)(parameters, lr=learning_rate)
        ...
        for epoch in range(epochs):
            ...
            for images, targets in iterable:
                ...
                if writer: writer.add_scalar("Loss/train", total_loss, epoch)
                ...
                optimizer.step()
            avg_train_loss = sum(losses_train)/len(losses_train)
            
            if verbose: print("Epoch: ", epoch, " "*10, "Training Loss: ", avg_train_loss)
            # Validation step
            if val_dataset is not None:
                ...
                if writer: writer.add_scalar("Loss/val", avg_loss, epoch)
...
def preprocessing_image_id(df):
    filename_id_mapping = {v:k for k,v in dict(enumerate(df['image_id'].unique())).items()}
    df["image_id"] = df['image_id'].apply(lambda x: filename_id_mapping[x])
    return df